      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-5xl">
        {devices.map((device) => {
          ...
          return (
            <div 
              key={device.name}
              className="bg-[#1a1c22] border border-white/10 rounded-lg py-4 px-8 flex items-center justify-center gap-3 hover:bg-[#252830] transition-all cursor-pointer group relative shadow-lg"
            >
              {device.badge && (
                <div className="absolute top-2 right-2 w-2 h-2 bg-red-600 rounded-full shadow-[0_0_8px_rgba(220,38,38,0.5)]"></div>
              )}
