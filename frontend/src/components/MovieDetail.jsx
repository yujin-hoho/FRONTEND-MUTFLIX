...
export default function MovieDetail({ session, activeProfile }) {
    ...
    <>
      <div className="min-h-screen bg-[#141414] text-slate-100 flex flex-col animate-fadeIn">
        <div className="flex-1 w-full space-y-0">
              ...
              <div className="bg-[#181818] border border-white/10 p-6 rounded-lg space-y-6">
                ...
                {isVideosLoading ? (
                  ...
                  <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                    {selectedItemVideos.map((video, idx) => {
          ...
          {/* Cast & Crew Section - Directly below Hero/Details Grid */}
          <div className="space-y-8">
            {/* Cast List */}
            {cast.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-white border-b border-slate-900 pb-2">Cast</h3>
            ...
            {crew.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-white border-b border-slate-900 pb-2">Crew</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4">
                  {crew.filter(c => ['Director', 'Producer', 'Screenplay', 'Writer'].includes(c.job)).slice(0, 8).map((member, idx) => (
